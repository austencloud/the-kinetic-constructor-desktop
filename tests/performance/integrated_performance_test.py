...
try:
    from browse_tab_v2.startup.performance_preinitialization import (
...
class IntegratedPerformanceTest:
    ...
    def test_widget_creation_before_and_after(self):
        ...
        self.logger.info("=== TESTING WIDGET CREATION: BEFORE vs AFTER PRE-INITIALIZATION ===")
        ...
        if preinitialization_results["overall_success"]:
            self.logger.info(f"✅ Pre-initialization successful in {preinitialization_results['overall_duration_ms']:.1f}ms")
        else:
    ...
    def test_viewer_initialization_before_and_after(self):
        ...
        self.logger.info("=== TESTING VIEWER INITIALIZATION: BEFORE vs AFTER PRE-INITIALIZATION ===")
        ...
        for i in range(2):
            ...
            try:
                ...
                self.logger.error(f"Viewer initialization before test {i+1} failed: {e}")
    ...
    def run_integrated_tests(self):
        ...
        self.logger.info("🔄 Starting Integrated Performance Tests (Before/After Pre-initialization)...")
    ...
    def generate_integrated_report(self):
        ...
        significant_improvement_count = sum(1 for r in self.results if r.significant_improvement)
        
        self.logger.info(f"Total Tests: {total_tests}")
        self.logger.info(f"Targets Achieved: {target_achieved_count}/{total_tests}")
        self.logger.info(f"Significant Improvements: {significant_improvement_count}/{total_tests}")
        ...
        for result in self.results:
            target_status = "✅ TARGET MET" if result.target_achieved else "❌ TARGET MISSED"
            improvement_status = "🚀 SIGNIFICANT" if result.significant_improvement else "⚠️ MINIMAL"
            
            self.logger.info(f"{result.test_name.upper()}:")
            self.logger.info(f"  Before Pre-init: {result.before_preinitialization_ms:.1f}ms")
            self.logger.info(f"  After Pre-init:  {result.after_preinitialization_ms:.1f}ms")
            self.logger.info(f"  Improvement:     {result.improvement_factor:.1f}x faster")
            self.logger.info(f"  Target:          {result.target_ms:.1f}ms ({target_status})")
        ...
        if target_achieved_count == total_tests and significant_improvement_count == total_tests:
            ...
            self.logger.info("   Some improvements achieved, but targets may not be met")
        else:
            self.logger.info("❌ INEFFECTIVE: Pre-initialization not providing expected benefits")
        ...
        if widget_results:
            ...
            if not widget_result.target_achieved:
                ...
                self.logger.info(f"   Current: {widget_result.after_preinitialization_ms:.1f}ms, Target: {widget_result.target_ms:.1f}ms")
        ...
        if viewer_results:
            ...
            if not viewer_result.target_achieved:
                ...
                self.logger.info(f"   Current: {viewer_result.after_preinitialization_ms:.1f}ms, Target: {viewer_result.target_ms:.1f}ms")
                self.logger.info(f"   Recommendations:")
                self.logger.info(f"   - Pre-create more animation manager instances")
                self.logger.info(f"   - Warm up animation property system more thoroughly")
