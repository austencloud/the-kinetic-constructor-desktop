...
class PreinitializationValidationTest:
    ...
    def run_preinitialization(self) -> bool:
        ...
        if results["overall_success"]:
            self.logger.info(f"✅ Pre-initialization successful in {results['overall_duration_ms']:.1f}ms")
            for system_name, system_result in results["systems"].items():
                status = "✅" if system_result["success"] else "❌"
                self.logger.info(f"  {status} {system_name}: {system_result['duration_ms']:.1f}ms")
        else:
    ...
    def validate_system_states(self):
        ...
        for system, ready in validation_results.items():
            ...
            self.logger.info(f"  {status} {system}: {'Ready' if ready else 'Not Ready'}")
    ...
    def test_widget_creation_performance(self):
        ...
        if creation_times:
            ...
            improvement_status = "🚀 IMPROVED" if improvement_achieved else "⚠️ NO IMPROVEMENT"
            
            self.logger.info(f"{status} Widget creation: avg={avg_time:.1f}ms, max={max_time:.1f}ms "
    ...
    def test_viewer_initialization_performance(self):
        ...
        if init_times:
            ...
            self.logger.info(f"{status} Viewer initialization: avg={avg_time:.1f}ms, max={max_time:.1f}ms "
                           f"(target: <{expected_max}ms)")
    ...
    def generate_validation_report(self, preinitialization_success: bool):
        ...
        self.logger.info(f"Performance Improvements: {improved_tests}/{total_tests} achieved")
        ...
        for result in self.results:
            ...
            improvement = "🚀 IMPROVED" if result.improvement_achieved else "⚠️ NO IMPROVEMENT"
            
            self.logger.info(f"{status} {result.test_name}:")
            self.logger.info(f"  Performance: {result.actual_ms:.1f}ms (target: <{result.expected_max_ms}ms)")
            self.logger.info(f"  Status: {improvement}")
            
            if "all_times" in result.details:
                times = result.details["all_times"]
                self.logger.info(f"  Consistency: {min(times):.1f}ms - {max(times):.1f}ms")
                
        # Overall assessment
        self.logger.info("\nOVERALL ASSESSMENT:")
        self.logger.info("-" * 30)
        
        if preinitialization_success and passed_tests == total_tests and improved_tests == total_tests:
            self.logger.info("🎉 EXCELLENT: Pre-initialization working perfectly!")
            self.logger.info("   All performance targets achieved with significant improvements")
