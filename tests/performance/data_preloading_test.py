...
try:
    from browse_tab_v2.startup.data_preloader import (
...
class DataPreloadingTest:
    ...
    def test_data_preloading_performance(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"{status} Data pre-loading: {preloading_duration:.1f}ms "
    ...
    def test_navigation_sidebar_preloading(self):
        ...
        try:
            ...
            status = "✅ PASS" if result.target_met else "❌ FAIL"
            self.logger.info(f"{status} Navigation sidebar: {creation_duration:.1f}ms "
    ...
    def test_browse_tab_view_preloading(self):
        ...
        if not COMPONENTS_AVAILABLE:
            self.logger.warning("Components not available, skipping browse tab view tests")
            return
        ...
        try:
            ...
            has_navigation = hasattr(browse_tab, "navigation_sidebar") and len(browse_tab.navigation_sidebar.sections) > 0
            ...
            result = PreloadingTestResult(
                ...
                details={
                    ...
                    "navigation_sections": len(browse_tab.navigation_sidebar.sections) if has_navigation else 0,
            ...
            status = "✅ PASS" if result.target_met else "❌ FAIL"
            self.logger.info(f"{status} Browse tab view: {creation_duration:.1f}ms "
    ...
    def test_preloaded_data_integrity(self):
        ...
        if not COMPONENTS_AVAILABLE:
            self.logger.warning("Components not available, skipping data integrity tests")
            return
        ...
        try:
            ...
            data_valid = (
                isinstance(sequences, list) and
                isinstance(navigation_sections, dict) and
                isinstance(thumbnail_cache, dict) and
                isinstance(metadata_cache, dict)
            )
            
            # Check data completeness
            has_sequences = len(sequences) > 0
            has_alphabetical_sections = "alphabetical" in navigation_sections and len(navigation_sections["alphabetical"]) > 0
            ...
            result = PreloadingTestResult(
                ...
                details={
                    ...
                    "navigation_sections_count": sum(len(sections) for sections in navigation_sections.values()),
            ...
            status = "✅ PASS" if result.target_met else "❌ FAIL"
            self.logger.info(f"{status} Data integrity: sequences={len(sequences)}, "
    ...
    def generate_preloading_report(self):
        ...
        for result in self.results:
            ...
            display_status = "⚡ IMMEDIATE" if result.immediate_display else "⏳ DELAYED"
            
            self.logger.info(f"{status} {result.test_name}:")
            
            if result.preloading_duration_ms > 0:
                self.logger.info(f"  Pre-loading: {result.preloading_duration_ms:.1f}ms")
            if result.component_creation_ms > 0:
                self.logger.info(f"  Component Creation: {result.component_creation_ms:.1f}ms")
        ...
        if preloading_tests:
            avg_preloading = sum(r.preloading_duration_ms for r in preloading_tests) / len(preloading_tests)
        ...
        if component_tests:
            avg_component = sum(r.component_creation_ms for r in component_tests) / len(component_tests)
