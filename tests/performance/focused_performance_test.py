...
class FocusedPerformanceTest:
    ...
    def test_widget_creation_performance(self):
        ...
        for test_name, beat_count in test_cases:
            ...
            try:
                ...
                self.logger.info(f"{status} Widget creation {test_name}: {duration_ms:.1f}ms (target: 50ms)")
        ...
        for sequence in test_sequences:
            ...
            try:
                ...
                init_result = PerformanceResult(
                    ...
                    details={"operation": "initialization", "beat_count": sequence.length}
                )
                ...
                display_result = PerformanceResult(
                    ...
                    details={"operation": "display_sequence", "beat_count": sequence.length}
                )
                ...
                self.logger.info(f"{init_status} Viewer init {sequence.name}: {init_duration:.1f}ms (target: 100ms)")
                self.logger.info(f"{display_status} Viewer display {sequence.name}: {display_duration:.1f}ms (target: 200ms)")
                
            except Exception as e:
                self.logger.error(f"Sequence viewer test failed for {sequence.name}: {e}")
        ...
        try:
            ...
            for op_name, operation in test_operations:
                ...
                try:
                    ...
                    self.results.append(result)
                    status = "✅ PASS" if result.passed else "❌ FAIL"
                    self.logger.info(f"{status} Cache {op_name}: {duration_ms:.1f}ms (target: 50ms)")
    ...
    def generate_performance_report(self):
        ...
        for category, results in categories.items():
            ...
            self.logger.info(f"{category.upper()}: {passed}/{total} passed, avg: {avg_time:.1f}ms")
        ...
        if widget_tests:
            avg_widget_time = sum(r.duration_ms for r in widget_tests) / len(widget_tests)
            if avg_widget_time > 50:
                self.logger.info(f"🔧 WIDGET CREATION OPTIMIZATION NEEDED:")
                self.logger.info(f"   Current average: {avg_widget_time:.1f}ms, Target: 50ms")
                ...
                self.logger.info(f"   - Implement lazy loading for non-critical elements")
                
        viewer_tests = [r for r in self.results if 'viewer' in r.test_name]
        if viewer_tests:
            avg_viewer_time = sum(r.duration_ms for r in viewer_tests) / len(viewer_tests)
            if avg_viewer_time > 150:
                self.logger.info(f"🔧 SEQUENCE VIEWER OPTIMIZATION NEEDED:")
                self.logger.info(f"   Current average: {avg_viewer_time:.1f}ms, Target: 150ms")
                self.logger.info(f"   Recommendations:")
