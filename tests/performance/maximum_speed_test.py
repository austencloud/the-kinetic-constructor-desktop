...
class MaximumSpeedTest:
    ...
    def run_widget_creation_speed_test(self, iterations: int = 20):
        ...
        self.logger.info(f"=== WIDGET CREATION SPEED TEST ({iterations} iterations) ===")
        ...
        for test_name, beat_count in test_cases:
            ...
            for i in range(iterations):
                ...
                try:
                    ...
                    if i == 0:
                        self.logger.info(f"  First run {test_name}: {duration_ms:.1f}ms")
                    elif i % 5 == 0:
                        recent_avg = sum(times[-5:]) / min(5, len(times))
                        self.logger.info(f"  Iteration {i}: {duration_ms:.1f}ms (recent avg: {recent_avg:.1f}ms)")
            ...
            if len(times) > 1:
                ...
                result = SpeedTestResult(
                    ...
                    steady_state_std_ms=statistics.stdev(steady_state) if len(steady_state) > 1 else 0,
                    target_ms=50.0,
                ...
                self.logger.info(f"{status} Widget {test_name}: {result.steady_state_avg_ms:.1f}ms avg "
    ...
    def run_sequence_viewer_speed_test(self, iterations: int = 15):
        ...
        self.logger.info(f"=== SEQUENCE VIEWER SPEED TEST ({iterations} iterations) ===")
        ...
        for i in range(iterations):
            try:
                ...
                if i == 0:
                    self.logger.info(f"  First run init: {init_duration:.1f}ms, display: {display_duration:.1f}ms")
                elif i % 3 == 0:
                    recent_init_avg = sum(init_times[-3:]) / min(3, len(init_times))
                    recent_display_avg = sum(display_times[-3:]) / min(3, len(display_times))
                    self.logger.info(f"  Iteration {i}: init {init_duration:.1f}ms, display {display_duration:.1f}ms "
        ...
        if len(init_times) > 1:
            ...
            init_result = SpeedTestResult(
                ...
                steady_state_std_ms=statistics.stdev(steady_init) if len(steady_init) > 1 else 0,
                target_ms=100.0,
        ...
        if len(display_times) > 1:
            ...
            display_result = SpeedTestResult(
                ...
                steady_state_std_ms=statistics.stdev(steady_display) if len(steady_display) > 1 else 0,
                target_ms=200.0,
    ...
    def run_rapid_fire_test(self, iterations: int = 50):
        ...
        for i in range(iterations):
            ...
            if i % 10 == 0:
                ...
                self.logger.info(f"  Rapid fire {i}: {duration_ms:.1f}ms (recent avg: {recent_avg:.1f}ms)")
        ...
        self.logger.info(f"🚀 Rapid fire complete: {throughput:.1f} widgets/second, "
    ...
    def generate_speed_report(self):
        ...
        for result in self.results:
            ...
            self.logger.info(f"  Steady State: {result.steady_state_avg_ms:.1f}ms ± {result.steady_state_std_ms:.1f}ms")
            self.logger.info(f"  Range: {result.steady_state_min_ms:.1f}ms - {result.steady_state_max_ms:.1f}ms")
            self.logger.info(f"  Target: {result.target_ms:.1f}ms")
            self.logger.info(f"  Improvement: {result.improvement_factor:.1f}x faster after warmup")
            self.logger.info("")
        ...
        if widget_results:
            avg_widget_speed = statistics.mean([r.steady_state_avg_ms for r in widget_results])
            ...
            self.logger.info(f"   Maximum Improvement: {max_improvement:.1f}x after warmup")
        ...
        if viewer_results:
            for result in viewer_results:
                ...
                self.logger.info(f"   Improvement: {result.improvement_factor:.1f}x after warmup")
        ...
        if rapid_results:
            ...
            self.logger.info(f"   Consistent Speed: {rapid.steady_state_avg_ms:.1f}ms ± {rapid.steady_state_std_ms:.1f}ms")
