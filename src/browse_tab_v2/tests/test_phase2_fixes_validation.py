    ...
    try:
        ...
        for filter_type in FilterType:
            ...
            if actual_value == expected_value:
                ...
                print(f"⚠ {filter_type.name} = '{actual_value}' (expected '{expected_value}')")
        ...
        for attr_name, expected_type, expected_value in required_attrs:
            if hasattr(config, attr_name):
                ...
                if actual_type == expected_type and actual_value == expected_value:
                    ...
                    print(f"⚠ {attr_name}: {actual_value} ({actual_type.__name__}) - expected {expected_value} ({expected_type.__name__})")
            else:
...
def test_component_imports():
    ...
    components_to_test = [
        ("ResponsiveThumbnailGrid", "src.browse_tab_v2.components.responsive_thumbnail_grid"),
        ("ModernThumbnailCard", "src.browse_tab_v2.components.modern_thumbnail_card"),
...
def test_filter_chip_text_formatting():
    ...
    try:
        ...
        for filter_type, value, operator, expected_text in test_cases:
            ...
            elif filter_type == FilterType.FAVORITES and "Favorites Only" in formatted_text:
...
def test_quick_filter_logic():
    ...
    try:
        ...
        from src.browse_tab_v2.core.interfaces import FilterType
        ...
        if after_difficulty > before_difficulty:
            ...
            if last_filter.filter_type == FilterType.DIFFICULTY:
                ...
                print(f"✓ High difficulty filter value: {last_filter.value} (should be 4)")
                print(f"✓ High difficulty filter operator: {last_filter.operator}")
            else:
                print(f"⚠ High difficulty filter has wrong type: {last_filter.filter_type}")
        else:
