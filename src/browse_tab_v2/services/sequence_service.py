...
class SequenceService:
    ...
    def _find_thumbnails(self, word_path: str) -> List[str]:
        ...
        try:
            ...
            logger.info(f"🖼️ FIND_THUMBNAILS: Searching in '{word_path}'")
            ...
            for file_name in os.listdir(word_path):
                if file_name.lower().endswith(".png"):
                    ...
                    logger.info(f"🖼️ FIND_THUMBNAILS: Found thumbnail '{file_name}'")

            # Sort thumbnails to ensure consistent ordering
            thumbnails.sort()

            logger.info(
                f"🖼️ FIND_THUMBNAILS: Found {len(thumbnails)} thumbnails in '{word_path}'"
            )
