...
class LayoutCoordinator:
    ...
    def switch_to_stack_layout(self, left_stretch: int = 1, right_stretch: int = 1) -> None:
        """
        ...
        """
        self.logger.debug(f"Switching to stack layout with stretch {left_stretch}:{right_stretch}")
        
        if self._current_layout_mode != LayoutMode.STACK:
            self.coordinator.main_content_stack.setCurrentIndex(0)  # Show stack container
            self._current_layout_mode = LayoutMode.STACK
    ...
    def switch_to_full_widget_layout(self, tab_widget: QWidget) -> None:
        ...
        """
        self.logger.debug(f"Switching to full widget layout for {tab_widget.__class__.__name__}")
        
        # Check if this tab widget is already in the main content stack
        tab_index = -1
        for i in range(1, self.coordinator.main_content_stack.count()):  # Skip index 0 (stack container)
        ...
        if tab_index == -1:
            ...
            self.logger.debug(f"Added tab widget to stack at index {tab_index}")
    ...
    def setup_initial_layout(self) -> None:
        ...
        self.coordinator.content_layout.addWidget(self.coordinator.left_stack, 1)  # Left stack
        self.coordinator.content_layout.addWidget(self.coordinator.right_stack, 1)  # Right stack
