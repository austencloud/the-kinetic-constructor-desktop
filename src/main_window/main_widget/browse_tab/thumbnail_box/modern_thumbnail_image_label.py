"""
Modern Thumbnail Image Label - Clean, component-based thumbnail display.

This is a complete rewrite using focused components following SRP:
...
class ModernThumbnailImageLabel(QLabel):
    """
    Modern thumbnail image label with component-based architecture.
    ...
    def update_thumbnail_async(self, index: int) -> None:
        ...
        if self._lazy_loading_coordinator.is_lazy_loading_enabled:
            success = self._lazy_loading_coordinator.request_image_load(
                path, target_size, self._on_image_loaded
            )
            if success:
                return

        # Fallback to regular async loading
        self._image_loading_manager.load_image_async(path, target_size, index)
    ...
    def _on_image_loaded(self, image_path: str, pixmap: QPixmap) -> None:
        """Handle image loaded from any source."""
    ...
    def _on_fallback_triggered(self, image_path: str) -> None:
        ...
        target_size = self._calculate_available_space()
        self._image_loading_manager.load_image_async(image_path, target_size)
    ...
    def force_reload(self) -> None:
        ...
        if self.current_path:
            target_size = self._calculate_available_space()
            self._image_loading_manager.load_image_async(self.current_path, target_size)
