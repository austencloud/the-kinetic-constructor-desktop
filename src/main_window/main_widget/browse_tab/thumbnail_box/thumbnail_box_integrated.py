"""
Thumbnail Box with Integrated Glassmorphism System.
...
from .thumbnail_image_label_integrated import ThumbnailImageLabelIntegrated

if TYPE_CHECKING:
    from main_window.main_widget.browse_tab.browse_tab import BrowseTab


class ThumbnailBoxIntegrated(QWidget):
    """
    Thumbnail box with integrated glassmorphism styling.
    ...
    def _setup_components(self):
        ...
        # Use integrated image label with maximum display
        self.image_label = ThumbnailImageLabelIntegrated(self)
    ...
    def _setup_layout(self):
        ...
        # CRITICAL FIX: Strict size policy to prevent vertical overflow
        self.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)

        self.setContentsMargins(0, 0, 0, 0)
        layout = QVBoxLayout(self)
        layout.setSpacing(2)  # Minimal spacing to prevent overflow
        layout.addWidget(self.header)

        # CRITICAL FIX: Image label with proper constraints
        layout.addWidget(self.image_label, 0)  # No stretch factor to prevent expansion
        self.image_label.setSizePolicy(
            QSizePolicy.Policy.Expanding,
            QSizePolicy.Policy.Fixed,  # Fixed height policy
        )

        layout.addWidget(self.nav_buttons_widget)
        # CRITICAL FIX: Remove addStretch() that causes vertical expansion
    ...
    def _apply_glassmorphism_styling(self):
        ...
        self.setObjectName("thumbnail_enhanced")
    ...
    def resize_thumbnail_box(self):
        ...
        if self.in_sequence_viewer:
            ...
            if columns > 0:
                ...
                width = 180  # Fallback width when no columns calculated

        # Update preferred width and apply
        self._preferred_width = width
        self.setFixedWidth(width)

        # CRITICAL FIX: Set maximum height to prevent overflow
        self.setMaximumHeight(220)  # Strict height limit
