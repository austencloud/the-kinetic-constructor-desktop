...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QCheckBox, QGroupBox, QScrollArea, QFrame
)
...
class ExportTestComponent(QWidget):
        ...
        try:
            ...
            for i, entry in enumerate(sequence):
                if isinstance(entry, dict):
                    ...
                    self.log_debug(f"  Entry {i}: {keys[:5]}{'...' if len(keys) > 5 else ''}")
                else:
    ...
    def test_start_position_only(self):
        ...
        try:
            ...
            settings_manager = self.main_widget.app_context.settings_manager
            settings_manager.image_export.set_image_export_setting("include_start_position", True)
            ...
            if len(full_sequence) >= 2:
                ...
                self.log_debug(f"Created minimal sequence with {len(minimal_sequence)} entries")
    ...
    def analyze_sequence_data(self):
        ...
        try:
            ...
            if len(sequence) > 1:
                ...
                self.log_debug(f"Entry 1 (start pos): {entry1_keys}, has_start_pos={has_start_pos}")
            ...
            if beats_count > 0:
                for i in range(2, min(len(sequence), 7)):  # Show first 5 beats
                    ...
                    self.log_debug(f"  Beat {i-2}: placeholder={is_placeholder}, has_letter={has_letter}")
